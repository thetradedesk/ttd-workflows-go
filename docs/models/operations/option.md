...
```go
ttdworkflows.WithServerURL("https://api.example.com")
```
...
```go
ttdworkflows.WithTemplatedServerURL("https://{host}:{port}", map[string]string{
...
```go
ttdworkflows.WithServer("my-server")
```
...
```go
ttdworkflows.WithClient(httpClient)
```
...
```go
ttdworkflows.WithSecuritySource(/* ... */)
```
...
```go
ttdworkflows.WithRetryConfig(retry.Config{
    Strategy: "backoff",
...
```go
ttdworkflows.WithTimeout(30 * time.Second)
```
